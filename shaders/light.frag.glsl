...
{       
  ...
  vec4 cur_diff = diffuse;
  vec4 cur_spec = specular;
  if (istex == 1)
  {
    cur_diff = texture2D(mytex, mytexcoord);
  ...
  vec4 col0 = ComputeLight(direction0, light0color, normal, half0, cur_diff, cur_spec, shininess) ;
